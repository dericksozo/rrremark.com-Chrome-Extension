function cre(base, opts, children) {
    ...
    if (base && base.cloneNode) {
        ...
        throw new TypeError('base must be a String, something with cloneNode, or falsy');
    }
    if (children) {
        if (typeof children == 'string' || children.length == 1 && typeof children[0] == 'string') {
            if (elem) {
    ...
    for (var opt in opts) {
        if (Object.prototype.hasOwnProperty.call(opts, opt)) switch (opt) {
            ...
            case 'attributes':
                ...
                break;
            case 'namespaceURI':
                // read-only, used during element creation
                break;
            default:
                elem[opt] = opts[opt];
                break;
